...
## Description of Data
(description from [here](http://archive.ics.uci.edu/ml/datasets/Human+Activity+Recognition+Using+Smartphones) and readme.txt of data set)
...
The following files are available for both the train and test data. Their descriptions are equivalent.
...
All the values in the tidyData file are averages, aggregated over 30 Subjects and 6 Activities. 

There are 5 parts to this transformation:
...
#### How ```run_analysis.R``` Transforms the Data:

* Requires ```dplyr``` and ```data.table``` libraries.
* Loads both test and train data. 
* Loads the features and activity labels.
* Merges the test and train data sets. 
* Creates descriptive names for variables and activities. 
